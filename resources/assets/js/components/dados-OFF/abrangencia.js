class Abrangencia extends React.Component{
    constructor(props){
        ...
        this.state = {
            ...
            options: [
                ...
                {id: 3, title: 'Piauí', plural: 'os Estados', on:false, listAll:1, height: '400px'},
                {id: 7, title: 'Territorios', plural: 'os Territorios', on:false, listAll:1, height: '400px'},
                {id: 4, title: 'Municípios', plural: 'os Municípios', on:false, listAll:0, height: '400px',
                    filter:[
                        {id: 12, title: 'Acre'},
                        {id: 27, title: 'Alagoas'},
    ...
    render(){
        ...
        let options = this.state.options.map(function(item){
            return (
                <option key={"abr_"+item.id} value={item.id} disabled={!item.enable}>{item.title}</option>
            );
        }.bind(this));
        ...
        return(
            <div className="div-select" style={{float: 'left', marginRight: '5px'}}>
                <select className="form-control" onClick={this.check} style={{display:'inline'}}>
                    <option value="0">Todas as Abrangências</option>
                    {options}
                </select>
            </div>
        );
