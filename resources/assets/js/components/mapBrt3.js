class Map extends React.Component{
    ...
    loadMap(){
        ...
        for(let i in this.state.data.estacoes.features){
            ...
            for(let i in this.state.data.estacoes.features) {
                var polylinePoints = [
                    new L.LatLng(this.state.data.estacoes.features[i].geometry.coordinates[0], this.state.data.estacoes.features[i].geometry.coordinates[1]),
    ...
    refreshMarkers(data){
        ...
        for(let i in data['brt']["veiculos"]){
            ...
            L.marker([data["brt"]["veiculos"][i]["latitude"], data["brt"]["veiculos"][i]["longitude"]], {icon: marker}).bindPopup(
                '<b>'+data["brt"]["veiculos"][i]["trajeto"]+'</b><br>'+
                'Data Hora: '+data["brt"]["veiculos"][i]["dataHora"]+'<br>'+
                'Linha: '+data["brt"]["veiculos"][i]["linha"]+'<br>'+
                'Sentido: '+data["brt"]["veiculos"][i]["sentido"]+'<br>'+
                'Código: '+data["brt"]["veiculos"][i]["codigo"]+'<br>'+
                'Latitude: '+data["brt"]["veiculos"][i]["latitude"]+'<br>'+
                'Longitude: '+data["brt"]["veiculos"][i]["longitude"]+'<br>'
            ).addTo(markers);
