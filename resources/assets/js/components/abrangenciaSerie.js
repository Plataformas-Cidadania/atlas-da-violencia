class AbrangenciaSerie extends React.Component{
    ...
    render(){
        ...
        let btnContinuar = <button className="btn btn-primary" onClick={this.loadWithRegions}>{this.props.lang_continue}</button>
        ...
        let abrangencias = this.state.abrangencias.map(function(item){
            ...
            if(!this.state.abrangenciasOk.includes(item.id)){
                return (
                    <button key={'abrangencia'+item.id} className={"btn btn-type btn-type-inactive"} style={{marginLeft: '10px'}} >{item.titulo}</button>
                );
            }
            return (
                <button key={'abrangencia'+item.id} className={"btn btn-type "+(this.state.abrangencia == item.id ? "btn-type-active" : '')} style={{marginLeft: '10px'}} onClick={() => this.setAbrangencia(item.id)}>{item.titulo}</button>
            );
            ...
            <div>
                <div className="row">
                    {/*<div className="col-md-6">
                        <h2 style={{margin: 0}}>{tituloAbrangencia}</h2>
                    </div>*/}
                ...
                <Modal
                    ...
                    buttons={(
                        <div>
                            <button type="button" className="btn btn-default" data-dismiss="modal">{this.props.lang_cancel}</button>
                            {btnContinuar}
                        </div>
                    )}
                />
