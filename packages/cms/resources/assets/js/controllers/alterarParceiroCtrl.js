cmsApp.controller('alterarParceiroCtrl', ['$scope', '$http', 'Upload', '$timeout', function($scope, $http, Upload, $timeout){
    ...
    $scope.alterar = function (file, arquivo){
        ...
        if(file==null && arquivo==null){
            ...
            $http.post("cms/alterar-parceiro/"+$scope.id, {
                'parceiro': $scope.parceiro,
            ...
            Upload.upload({
                url: 'cms/alterar-parceiro/'+$scope.id,
                data: data1
                ...
                $scope.mensagemSalvar =  "Gravado com sucesso!";
                $scope.removerImagem = false;
                $scope.imagemBD = '/imagens/parceiros/'+response.data;
                $scope.processandoSalvar = false;
    ...
    $scope.carregaImagem  = function(img, arquivo) {
        if(img!=''){
            $scope.imagemBD = 'imagens/parceiros/xs-'+img;
