    ...
    render() {
        ...
        let temas = this.state.temas.map(function (item) {
            return React.createElement(
                'div',
                { key: "tema2_" + item.id, style: { float: 'left', padding: '15px', cursor: 'pointer' }, className: 'text-center', title: item.tema, onClick: () => this.select2(item.id) },
                ...
                React.createElement(
                    ...
                    item.tema.substr(0, 13).toLowerCase()
                )
            );
        }.bind(this));
