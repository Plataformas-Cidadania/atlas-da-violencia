class SelectItems extends React.Component {
    ...
    componentWillReceiveProps(props) {
        if (this.state.option != props.option || this.state.itemDefault != props.itemDefault) {
            let parameters = this.state.parameters;
            parameters.option = props.option;
            this.removeAll();
            this.setState({ option: props.option, parameters: parameters, itemDefault: props.itemDefault }, function () {
                this.loadData();
            });
        }
    }
    ...
    loadData() {
        ...
        $.ajax({
            ...
            data: {
                ...
                itemDefault: this.state.itemDefault
            },
            cache: false,
            success: function (data) {
                ...
                //console.log(':::', _return.items);

                if (this.state.itemDefault) {

                    this.setState({ items: _return.items, all: _return.all, itemsSelected: [_return.items[0]] }, function () {
                        this.setState({ loading: false });
                        this.select(_return.items[0].id);
                    });
                } else {
