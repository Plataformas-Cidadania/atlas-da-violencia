class Map extends React.Component {
    ...
    componentDidMount() {
        this.setState({ mymap: L.map(this.props.mapId).setView([-14, -52], 4) }, function () {
            let tileLayer = L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiYnJwYXNzb3MiLCJhIjoiY2l4N3l0bXF0MDFiczJ6cnNwODN3cHJidiJ9.qnfh8Jfn_be6gpo774j_nQ', {
                maxZoom: 18,
                attribution: '<div class="print-off" style="float:right;">&nbsp;Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, ' + '<a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, ' + 'Imagery © <a href="http://mapbox.com">Mapbox</a></div>',
                /*id: 'mapbox.streets'*/
                id: 'mapbox.streets'
    ...
    makeInfo() {
        this.setState({ info: L.control() }, function () {
            ...
            this.state.info.update = function (props) {
                ...
                this._div.innerHTML = '<h6 style="margin: 3px 0; font-weight: bold">' + _this.state.serie + '</h6>' + (props ? '<b>' + sigla + props.nome + '</b><br />' + formatNumber(props.total, _this.props.decimais, ',', '.') : _this.props.lang_mouse_over_region);
            };
    ...
    loadMap(data) {
        ...
        legend[indexLegend].onAdd = function (mymap) {
            ...
            for (let i = 0; i < grades.length; i++) {
                div.innerHTML += '<i style="background:' + getColor(grades[i] + 1, this.state.intervalos) + '"></i> ' + formatNumber(grades[i], this.props.decimais, ',', '.') + (grades[i + 1] ? '&nbsp;&ndash;&nbsp;' + formatNumber(grades[i + 1], this.props.decimais, ',', '.') + '<br>' : '+');
            }
            return div;
        }.bind(this);
        ...
        let polygon2 = L.polygon([[51.509, -0.08], [51.503, -0.06], [51.51, -0.047]]).addTo(this.state.mymap);
    ...
    render() {
        ...
        return React.createElement(
            ...
            React.createElement('div', { id: this.props.mapId, className: 'map' })
        );
    }
}