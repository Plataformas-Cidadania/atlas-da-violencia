                ...
                for (let j = 0; j < _this.state.regioes.length; j++) {
                    ...
                    for (let i = 0; i < testData.data.length; i++) {
                        if (testData.data[i].sigla_regiao == _this.state.regioes[j].sigla) {
                            ...
                            var m = L.marker(L.latLng(testData.data[i].lat, testData.data[i].lng), { icon: tipoIcon }).bindPopup('' + '<strong>' + testData.data[i].tipo + '</strong><hr style="margin:5px 0; padding:0;">' + '<strong>Data:</strong> ' + testData.data[i].data + '<br>' + '<strong>Turno:</strong> ' + testData.data[i].turno + ' <br>' + '<strong>Faixa Etária:</strong> ' + testData.data[i].faixa_etaria + ' <br>' + '<strong>Sexo:</strong> ' + testData.data[i].sexo + ' <br>' + '<strong>Tipo Acidente:</strong> ' + testData.data[i].tipo_acidente + ' ').openPopup();
                            regiao.addLayer(m);
                        }
                    }
